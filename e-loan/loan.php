...
<body>
      <div id="app">
            <!-- Modal -->
            <div class="modal fade" ref="modalsave" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                        ...
                        <transition name="fade">
                              <div class="error-info" v-show="error_display">
                                    <div class="alert alert-danger">
                                          <p v-for="error in error_count"> {{ error }} </p>
                                    </div>
                              </div>
                        </transition>
                  </div>
            ...
            new Vue({
                  ...
                  methods: {
                        ...
                        test_save() {
                              ...
                              vm.error_name = "";
                              var result = this.compute_loan();


                              if (!this.amount) {
                                    // error_name = error_name + "Please input the amount field \n";
                                    vm.error_count[0] = "Please input the amount field";
                                    vm.error_input.amount = true;
                                    error.amount = true;
                              } else {
                                    vm.error_count[0] = "";
                                    error.amount = false;
                              ...
                              if (!this.terms) {
                                    // error_name = error_name + "Please input the terms field \n";
                                    vm.error_count[1] = "Please input the terms field";
                                    vm.error_input.terms = true;
                                    error.terms = true;
                              } else {
                                    vm.error_count[1] = "";
                                    error.terms = false;
                              ...
                              if (!this.purpose) {
                                    // error_name = error_name + "Please input the amount field \n";
                                    vm.error_count[2] = "Please input the purpose field";
                                    vm.error_input.purpose = true;
                                    error.purpose = true;
                              } else {
                                    vm.error_count[2] = "";
                                    error.purpose = false;
                              ...
                              if (!this.firstcomaker) {
                                    // error_name = error_name + "Please input the terms field \n";
                                    vm.error_count[3] = "Please input the first-comaker field";
                                    vm.error_input.fcm_comaker = true;
                                    error.firstcomaker = true;
                              } else {
                                    vm.error_count[3] = "";
                                    error.firstcomaker = false;
                              }

                              if (!this.secondcomaker) {
                                    // error_name = error_name + "Please input the terms field \n";
                                    vm.error_count[4] = "Please input the second-comaker field";
                                    vm.error_input.scm_comaker = true;
                                    error.secondcomaker = true;
                              } else {
                                    vm.error_count[4] = "";
                                    error.secondcomaker = false;
                              }


                              if (error.amount === false && error.terms === false && error.purpose === false && error.firstcomaker === false && error.secondcomaker === false) {
                                    if (this.firstcomaker === this.secondcomaker) {
                                          vm.error_count[5] = "Invalid same comaker";
                                          vm.error_input.fcm_comaker = true;
                                          vm.error_input.scm_comaker = true;
                                          vm.error.firstcomaker = true;
                                    } else {
                                          vm.error_count[5] = "";
                                          ...
                                          axios({
                                                ...
                                                if (response.data === "error1") {
                                                      vm.error_count[6] = "First co-maker is not qualified";
                                                      vm.error_input.fcm_comaker = true;
                                                } else if (response.data === "error2") {
                                                      vm.error_count[6] = "Second co-maker is not qualified";
                                                      vm.error_input.scm_comaker = true;
                                                } else if (response.data === "errorall") {
                                                      vm.error_count[6] = "First and Second co-maker is not qualified";
                                                      vm.error_input.fcm_comaker = true;
                                                      vm.error_input.scm_comaker = true;
                                                } else if (response.data === "success") {
                                                      // vm.$refs.modalsave.modal('show');
                              ...
                              for (let index = 0; index < vm.error_count.length; index++) {
                                    vm.error_name = vm.error_name.concat(vm.error_count[index] + " ");
                              }

                              setTimeout(function() {
                                    vm.error_display = false;
                                    vm.error_input.terms = false;
                                    vm.error_input.amount = false;
                                    vm.error_input.terms = false;
                                    vm.error_input.purpose = false;
                        ...
                        compute_loan() {
                              ...
                              if (!this.amount || !this.terms) {
                                    ...
                                    this.compute.lnAmount = Math.round(this.amount);
                                    this.compute.terms = Math.round(this.terms);
                                    this.compute.interest = Math.round(this.compute.lnAmount * (this.compute.terms * this.compute.percent));
                                    this.compute.totalloanamount = Math.round(parseFloat(this.amount) + this.compute.interest);
                                    this.compute.monthlypayment = Math.round((this.compute.totalloanamount / this.terms));

                                    this.totalloanamount = Math.round(this.totalloanamount);
                                    this.compute.monthlypayment = Math.round(this.compute.monthlypayment);
                                    this.compute.interest = Math.round(this.compute.interest);
                                    this.compute.interest = Math.round(parseFloat(this.compute.interest));
                  ...
                  mounted() {
                        ...
                        vm.terms = 24;
                        vm.purpose = "wala";
                        vm.firstcomaker = 5104102;
                        vm.secondcomaker = 5107658;
                        }, 1000);
                  }
            }).$mount("#app");
      </script>
